...
select
  ...
  'Miles McQueen reports on ' || lower(category_name) || ' options for ' || lower(replace(use_case_name, 'For ', '')) || ', with ' || products[(i % 10) + 1] || ', ' || products[((i + 3) % 10) + 1] || ', rollout effort, cost fit, and a ' || (7 + (i % 9) * 2)::text || '-hour monthly improvement model in focus.',
  ...
  'After reviewing the workflow around ' || pain || ', the strongest ' || lower(category_name) || ' shortlist for ' || replace(use_case_name, 'For ', '') || ' centers on ' || products[(i % 10) + 1] || ', ' || products[((i + 2) % 10) + 1] || ', and ' || products[((i + 5) % 10) + 1] || '. The deciding issue is ' || metric || ': teams should expect about ' || (8 + ((i * 3) % 17))::text || ' hours returned, a ' || (11 + ((i * 5) % 19))::text || ' percent operating lift, and a monthly budget near $' || (260 + ((i * 73) % 940))::text || ' before add-ons.',
